...
| 4 | Restarted PDHG — **CPU done**, its answer finished by the interior point by default since #229; CUDA backend on `main` (`src/gpu/`, #329 to #373), compiled by the CUDA CI job and run on no card yet, so there is no GPU number: #19 is the measurement | partial |
...
src/pdhg          restarted PDHG, CPU; its answer is finished by the interior point (#229)
                                                       (CUDA backend: src/gpu, on main, unmeasured; #19)
...
|---|---|---|
| **CUDA backend for PDHG** | [#16](https://github.com/thegoodengineers/SANKHYA/issues/16), [#17](https://github.com/thegoodengineers/SANKHYA/issues/17), [#19](https://github.com/thegoodengineers/SANKHYA/issues/19); PR [#390](https://github.com/thegoodengineers/SANKHYA/pull/390) | The "GPU-accelerated" in the problem statement's title. The plumbing (#16) and the kernels (#17) are on `main` and compile in the CUDA CI job; what is unwritten is the evidence (#19): the CPU-vs-GPU crossover by model size and the honest 1e-8 list, measured on a card and committed as a CSV. #390 is the first attempt at that CSV, sent back for a re-run because it timed the interior-point polish and the process start-up along with the solve. The measurement protocol is in [`docs/GPU_PLAN.md`](docs/GPU_PLAN.md). |
