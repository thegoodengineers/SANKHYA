...
It is a Smart India Hackathon team entry, and the competition requires the solver to be
built by the registered team alone, so outside pull requests and issue assignments cannot
be accepted; reading it, running the benchmarks, and reporting a number that does not
...
1g of `docs/BENCHMARKS.md`); on Mittelmann's brazil3 at 1e-8, 8.10x on a datacenter L4 and
10.53x on an A100 against the faster of that host's CPU arms (`gpu-real-{l4,a100}-58a8374.csv`,
section 1g.3), with a larger error than the CPU there (the GPU rows below).**
...
| 4 | Restarted PDHG - **CPU done**, its answer finished by the interior point by default since #229; CUDA backend on `main` (`src/gpu/`, #329 to #373), measured by #390 and re-run by #424 on an RTX 5050 (`bench/results/gpu-fb72ab4.csv`, `docs/BENCHMARKS.md` section 1g): slower than the CPU up to 5,000 x 5,000, 1.37x faster at 10,000 x 10,000 at 1e-4, PDHG alone on synthetic KKT LPs; on a datacenter NVIDIA L4 (E2E Networks TIR, fresh clone at `fdc89c5`, `bench/results/gpu-l4-fdc89c5.csv`, section 1g.3): **4.21x** at 10,000 x 10,000 at 1e-4 (3.90x at 1e-8) against that host's CPU PDHG on one thread (this synthetic ladder has not been re-run with the multithreaded arm), 2.05x at 5,000, still a loss at 2,000. On real models, fresh clone at `58a8374`, against the faster of the two CPU arms (`gpu-real-l4-58a8374.csv`, `gpu-real-a100-58a8374.csv`): on the L4 the 16-thread arm is slower than one thread, so brazil3 is **5.75x** at 1e-4 and **8.10x** at 1e-8 and chromaticindex1024-7 2.92x / 2.88x, all against one thread; on an A100 brazil3 **8.91x** / **10.53x** and chromaticindex1024-7 2.38x / 1.77x. At 1e-8 on brazil3 the GPU's answer is further from HiGHS than the CPU's, and on the 779,640-row refinery year every arm on both cards hits the 300 s limit. H100 not yet | done, measured on a laptop card and two datacenter cards (L4, A100) |
| 5 | Branch & bound → MILP | **done** (MIPLIB benchmarked; root cuts landed in #159 and are on by default since the three-way at `0018254`, see below) |
...
| measurement | count |
|---|---|
| matched Netlib's published optimum to a relative 1e-6 **and** passed independent verification by `tools/verify_solution.py` | **81 of 89** |
| returned an optimum that the independent verifier accepts | **89 of 89** |
...
with an answer that checks out. The gap between them is the eight instances where the readme
is the one that is off; at `5b2bd80` nothing is left to the clock (below). The narrower tiers read higher
...
results*, Operations Research Letters 32, 2004; the values below are from the table in the
report version, ZIB-Report 03-05, 2003). Against those exact values:

| instance | ours (`5b2bd80`) | Netlib readme | Koch, exact | ours vs exact | readme vs exact |
|---|---:|---:|---:|---:|---:|
| `80bau3b` | 987224.19240909 | 987232.16072 | 987224.192409090 | 9.4e-16 | 8.1e-06 |
| `e226` | -11.638929066371, and -18.751929066371 with the objective constant (below) | -18.751929066 | -18.7519290663705 | 2.0e-15 with the constant | 2.0e-11 |
| `ganges` | -109585.73612928 | -109586.36356 | -109585.736129278 | 2.0e-15 | 5.7e-06 |
| `greenbea` | -72555248.129846 | -72462405.908 | -72555248.1298460 | 2.1e-16 | 1.3e-03 |
| `greenbeb` | -4302260.2612066 | -4302147.6065 | -4302260.26120659 | 8.7e-16 | 2.6e-05 |
| `nesm` | 14076036.487563 | 14076073.035 | 14076036.4875627 | 7.9e-16 | 2.6e-06 |
| `pilot` | -557.48972928933 | -557.40430007 | -557.489729284068 | 9.4e-12 | 1.5e-04 |
| `scrs8` | 904.29695380079 | 904.29998619 | 904.296953800791 | 1.0e-15 | 3.4e-06 |

`e226` is different in kind: there the readme agrees with the exact value, and our
reported objective leaves out the constant 7.113 that the file carries on its objective
row (the CSV's `objective_offset` and `differs_by_objective_constant` columns). Adding it
back gives -18.751929066371, the exact value to 2.0e-15. That is a reporting convention to
settle, not a wrong answer. The other seven are the readme's error, which HiGHS, run as a
separate process by `bench/runners/cross_check_highs.py`
(`bench/results/cross-check-highs-6bec31e.csv`), also reports.

**One of the 81 passes the readme without being within 1e-6 of the exact optimum.**
`pilot87`: Koch's exact value is 301.710347333, ours is 301.710691459 (1.14e-6 relative),
and the readme's is 301.71072827 (1.26e-6). Our answer "matches" partly because the readme
is off in the same direction. It is counted in the 81 because that column is defined
...
than robustness. Tracked in #214 (`maros-r7`, since taken off the clock by the interior point, #466) and,
for the scale families, #279.

MIPLIB 2017 is benchmarked too: **13 of 30** easy instances reach the published optimum and
**10 of 30** also prove it under the shipped defaults (`bench/results/miplib-5daee10.csv`, 60 s,
on `main`, 24 September 2026: the plain re-run on the merged binary with root cuts on since
`0018254`; the cuts-on leg of the three-way, `bench/results/miplib-cuts-on.csv`, read the
same 13 and 10). Before root cuts the defaults read 14 and 9 (`bench/results/miplib-078cb24.csv`), the same 14 and 9 as `miplib-b3f1660.csv` after presolve
...
search (`--option mip_threads=N`, #403) and root cutting planes that are on by default. The
three-way A/B at `0018254`, alone on the machine with cut selection (#415) landed
(`bench/results/miplib-cuts-{off,on,tree}.csv`, `docs/BENCHMARKS.md` section 2): no cuts
reach 15 and prove 9; the root round reaches 13 and proves **10** at **1.015x** the nodes
and 0.986x the wall time, `neos-3611689-kaihu` 119 optimal in 57,576 nodes where it stopped
feasible at the limit without cuts; the two matches it does not repeat, `enlight8` and
`noswot`, are incumbents found at the limit that move between identical runs (`enlight8`
at 60 s has landed on 22, 23, 24, 27 and 28 across same-commit legs). Root **plus tree
rounds** at depth 4 reach 12 and prove 9 - the `neos-3611689-kaihu` proof lost again, 120
feasible in 35,996 nodes, fewer nodes in the same wall time because each node re-filters
the waiting pool - so `tree_cut_depth` stays 0. Before #415 the same root round cost
1.049x the nodes and lost the match; selection is what changed the default. Either way the
search finds good incumbents far more often than it closes the bound. For
...
**2 of 8** under `algorithm=auto` inside 300 s on `main` at `0018254`, alone on the machine
and on AC, every failure named in section 1d of `docs/BENCHMARKS.md`
(`bench/results/mittelmann-0018254.csv`): `brazil3` by the interior point in 3 s and
`chromaticindex1024-7` by PDHG in 83 s after the interior point declined on its setup share
and the solve fell back (#437). The same eight under the other two engines (#216,
`mittelmann-{pdhg,ipm}-0018254.csv`): PDHG alone finishes the same two, `chromaticindex1024-7`
in 2 s; the interior point finishes `brazil3` and **`qap15`** (226 s), which `auto` sends to
the dual simplex where it times out - so **3 of 8** across the three engines, and two of the
five that none reach are the interior point running out of memory on a 7.7 GB machine
(`bdry2`, `chromaticindex1024-7` under explicit `ipm`). Attribution of all eight is #417.
...
The same eight on a **96 GB** node (E2E Networks M3, 12-vCPU AMD EPYC 9555, `main` at
`5c7efbc`, `bench/results/mittelmann-{,pdhg-,ipm-}5c7efbc.csv`, #575): `auto` **2 of 8**,
PDHG **3 of 8** - `qap15` converges in 93.5 s where the laptop's PDHG left it at the limit -
the interior point **2 of 8**, **3 of 8** across engines. The memory bought the first-order
engine one instance and the interior point nothing: its six misses there are the AMD
ordering's own size budget (three rows, abandoned in 11 to 22 s) and the first
factorization (two rows), named row by row in #576, plus `irish-electricity` converged to
...
src/pdhg          restarted PDHG, CPU; its answer is finished by the interior point (#229)
                                                       (CUDA backend: src/gpu, on main, measured on three cards: 1.37x on a laptop RTX 5050, 4.21x on a datacenter L4, at 10,000 rows; brazil3 at 1e-8 8.10x on the L4, 10.53x on an A100, #390/#424/#488)
src/mip           branch and bound + diving heuristic + root cuts (on by default since `0018254`, #159, #415)
...
| limit | where it stands, and the issue that would move it |
|---|---|
| **GPU acceleration** - measured on one laptop card and two datacenter cards, the L4 and the A100; H100 not yet | The first-order method it needs exists and runs on CPU - restarted PDHG, `--option algorithm=pdhg`, **9 of 9** committed instances to `optimal` at 1e-8 on `main` at `4177ae6` (`bench/results/pdhg-4177ae6.csv`, `docs/BENCHMARKS.md` section 1e). The ninth is `share2b`, which the first-order method alone leaves at its million-iteration limit: since #229 its answer is finished by the interior point by default (`pdhg_polish`), nine more iterations there, and section 1f.1 measures the same thing size by size - 6.1e-04 to 1.9e-10 at 1,000 rows, declined where the factor is not affordable. The CUDA port is on `main` (`src/gpu/`, #329 to #373), compiles in CI, and was measured by #390 on an NVIDIA RTX 5050 Laptop GPU, PDHG alone on the solver's own clock at 1e-4 and 1e-8 on synthetic KKT LPs (`bench/results/gpu-fb72ab4.csv`, `docs/BENCHMARKS.md` section 1g): slower than the CPU up to 5,000 x 5,000, where the launch and transfer cost is not amortised, and **1.37x** faster at 10,000 x 10,000 at 1e-4 (1.54x at 1e-8). Two runs of the same measurement on the same card, #390 and the re-run in #424, put that 1e-4 crossover at 1.63x and 1.37x, so the figure carries run-to-run variance a single decimal place hides; the second is cited because it is the later of the two, not because it is the better one. `--gpu` on a build without CUDA warns and runs on the CPU; on a CUDA build it falls back to the CPU when the device, its compute capability or its memory fails the checks. The same protocol on a datacenter NVIDIA L4 (E2E Networks TIR, fresh clone at `fdc89c5`, `bench/results/gpu-l4-fdc89c5.csv`, section 1g.3): **4.21x** at 10,000 x 10,000 at 1e-4 (3.90x at 1e-8) against that host's CPU PDHG on one thread, 2.05x at 5,000, still a loss at 2,000; that synthetic ladder has not been re-run with the multithreaded CPU arm. The real models were re-run from a fresh clone at `58a8374` against two CPU arms, one thread and the parallel A x on 16 threads (L4) or 28 (A100), and each speed-up is quoted against the faster arm (`bench/results/gpu-real-l4-58a8374.csv`, `gpu-real-a100-58a8374.csv`, section 1g.3). On the L4 the 16-thread arm is the slower one on both Mittelmann models (brazil3 73.7 s against 37.6 s at 1e-4, 205.1 s against 90.8 s at 1e-8), so the card is measured against one thread: brazil3 **5.75x** at 1e-4 and **8.10x** at 1e-8 (6.5 s and 11.2 s on the card), chromaticindex1024-7 **2.92x** / **2.88x**. On an A100 (one card of a 2x A100-SXM4-40GB box): brazil3 **8.91x** / **10.53x** against one thread (52.1 s and 127.1 s against 5.8 s and 12.1 s; 28 threads took 61.2 s and 137.9 s), chromaticindex1024-7 **2.38x** / **1.77x** against 28 threads. The A100's larger brazil3 ratio comes from a slower host CPU (52.1 s against the L4 host's 37.6 s), not a faster card, and on chromaticindex1024-7 the A100 is slower than the L4 (0.59 s against 0.37 s at 1e-4). Where the GPU loses: at 1e-8 on brazil3 its answer is further from HiGHS than the CPU's, a relative gap of 2.4e-07 on the L4 and 1.3e-08 on the A100 against 1.2e-09 for both CPU arms, and that gap moves from run to run (9.15e-11 in the L4's median-of-three run, `gpu-datacenter-l4-58a8374.csv`, whose 1e-8 speed-up is 5.98x rather than 8.10x). On the 779,640-row refinery year every arm on both cards hits the 300 s limit (1.00x); the card is only further along, a gap of 3.7e-11 on the L4 and 5.5e-11 on the A100 against 3.6e-10 and 1.6e-08 for the better CPU arm. Measured beside it at `58a8374`, all opt-in and off by default: **cuDSS** for the interior point's normal equations (#489, a build with `-DSANKHYA_ENABLE_CUDSS=ON`, `ipm-cudss-ab-{mittelmann,netlib}-{cpu,cpu-supernodal,cudss}-58a8374.csv`) takes qap15 from the 300 s limit (scalar CPU factor) and 185.1 s (supernodal) to 18.9 s, solves rmine15 in 137.4 s where neither CPU factor does, and is slower than the scalar CPU factor on the smallest models (80bau3b 2.3 s against 1.2 s); **two A100s over NVLink** (#295, `multi-gpu-e2e-2xA100-SXM4-40GB-NV12-58a8374.csv`) are 1.64x faster per step at 4,000,000 rows, but the 2,000-step solve takes the same time (106.5 s against 108.4 s) because setup and host-side evaluation dominate, and two cards lose per step on the smaller Linf_520c (0.85x) and bdry2 (0.44x); **root domain propagation on the device** (#510, `gpu-domain-prop-58a8374.csv`) loses at every size, from 1422x slower at 1,000 rows to 1.13x slower at 1,000,000; **Feasibility Jump on the device** (#508, `gpu-fj-ab-58a8374-e2e-L4.csv`) finds a feasible point on 14 of 20 MIPLIB instances in 60 s against the CPU's 16 of 20; **batched PDHG for node bounds and strong branching** (#520, `miplib-t2-520-*.csv` against `miplib-t2-509-off.csv`) loses to leaving it off in every leg, with a weaker dual bound on 11 to 14 of the 20 instances. The crossover on those cards is the whole speed-up claimed. |
| **Scale** - no industrial model at size yet (#517) | Measured to 1,000,000 rows on generated instances only - no industrial model at that size - and the answer depends on the engine (#198, `docs/BENCHMARKS.md` sections 1f and 1f.1). On generated instances whose optimum is exact by construction: under a 120 s clock the first-order engine reaches the optimum at **100,000 x 100,000** to 1.1e-07 without certifying it and certifies it at 5,000 in 54 s, the interior point reaches it at 5,000 since the AMD ordering (#193) and the dual simplex only at 1,000 (`bench/results/scale-e134aeb.csv`, 7 of 12, measured on `main` after #242, #265 and #278). Given a fixed 1,000 iterations instead of a clock - the one accuracy claim here another machine reproduces exactly - the error stays between 6.9e-06 and 6.1e-04 all the way to **1,000,000 x 1,000,000**, so what grows with the model is the cost per iteration, not the number needed. Those are the random family's numbers, and a random sparse matrix is the worst case for anything that factorizes; on a second family shaped like a multi-period planning model (#198, section 1f.2, `bench/results/scale-staircase-e134aeb.csv`) the interior point reaches the optimum at **20,000** rows instead of 1,000 and 8 of 12 solves reach it against 7 of 12. The polish that finishes PDHG's answer (#229) is measured beside the unpolished run in section 1f.1 (`bench/results/scale-iterations-bf3df02.csv`): at 1,000 rows the same 1,000 iterations land at 6.1e-04 unpolished and 1.9e-10 polished, and at every larger size of the random family the polish declines within its 30 s budget because the factor is dense, which the table shows rather than hides. On real models: the largest Netlib instance solved is `fit2d`, 25x10500 with 129018 nonzeros, in 0.3 s, and on Mittelmann's eight smallest LPs, 6,330 to 376,500 rows, the result is 0 of 8 inside 300 s. On an industrial-structured model - a T-period refinery planning LP with crudes, tanks, yields, capacities, quality budgets and delivery commitments, optimum exact by construction (`bench/runners/generate_refinery_lp.py`, #211, section 1f.3, `bench/results/scale-refinery-e134aeb.csv`) - the monthly year (1,068 rows) is solved exactly by all three engines, the daily year (**32,485 rows**) exactly by the interior point (45 iterations, 68 s) and by PDHG finished by it (92 s), and the hourly year (**779,640 rows**) is reached by PDHG to 1.0e-06 inside 120 s and proved by nothing: 6 of 9 solves. That is the first industrial-shaped model at this size the evidence holds, and the number to read is the daily year. |
| **Interior point as a default** - the verifier-gated race (#476) landed, off by default; the learned selector is #477 | An interior-point method exists (#56, `--option algorithm=ipm`, Mehrotra predictor-corrector over a from-scratch sparse LDL^T) and since #353 the default `algorithm=auto` chooses it by rule: below 20,000 rows and 100,000 nonzeros the dual simplex runs (81 of 89 on the Netlib full set at `5b2bd80`, `bench/results/netlib-full-5b2bd80.csv`; on its own the interior point verifies 73 of 89 at `078cb24`, `bench/results/netlib-full-078cb24-ipm.csv`, up from 51 before #380's starting point and 69 before #389's regularization recovery; the 72 at `d8d434c` lost fit2p at the time limit on a slower pass), above either the interior point with crossover (maros-r7 solves in 9 s where the dual simplex hit the limit), from 100,000 rows PDHG; the answer carries the rule and the reason (`engine_rule`, `engine_reason`). The interior point still produces no basis of its own, so it is not the node engine. It stops the moment its iterate is not a number and returns the best finite one (#205), and it finishes PDHG's answers from PDHG's own point (#229), declining within `polish_max_seconds` when the factor is not affordable. On the random 20,000-row shape the interior point declines at a fifth of the time limit (#357) and PDHG takes the rest to a relative error of 8.8e-10; the three generated families under `auto` are `docs/BENCHMARKS.md` section 1f.4 (`bench/results/auto-scale-*-64d2e9a.csv`: 10 of 11 solves reach the analytic optimum). |
| **Cut rounds below the root** - root cuts ARE the default since `0018254`; tree rounds are not, measured to lose the proof they were meant to add (#221, `docs/NEGATIVE-RESULTS.md`) | Root Gomory mixed-integer, lifted knapsack cover and mixed-integer rounding cuts, and cut rounds below the root, exist (#159, #221; `--option enable_root_cuts=true`, `tree_cut_depth=4`) and the root round is on by default since `0018254`. On the 30-instance MIPLIB set at 60 s, alone on the machine, with cut selection (#415) landed: the root round proves **10** where no cuts prove 9, at 1.015x the node count and 0.986x the wall time, losing only limit-bound incumbents; root plus tree rounds at depth 4 lose that proof again in fewer, dearer nodes (`bench/results/miplib-cuts-{off,on,tree}.csv`; `docs/BENCHMARKS.md` section 2, with the root gap closed per instance). The tree leg stays opt-in, and the decision on #221 is made; what remains is the per-node cost of the waiting pool that makes tree rounds lose what the root gains. Row aggregation landed (#355) and clique and {0,1/2} cuts with a GF(2) separator followed (#379, #404); none of them separates anything on b-ball, opt1217, rlp1 or noswot, whose objective integrality (#398) moved the bound and whose plateau is now #418's. Branch and bound itself has reliability branching (#69) and warm-started dual node LPs (#65). This is why MIPLIB proves few optima. |
| **Non-convex QP and quadratic constraints** - a first slice of the global solver is #514, standard pooling instances #516 | Refused by default: a non-convex Hessian with an LDL^T certificate, a quadratic row (MPS `QCMATRIX`) by name at read time. A local optimum reported as a global one is not something this solver will do. `--option nonconvex=global` reads quadratic rows and solves continuous models by spatial branch and bound over McCormick relaxations (#514): `optimal` only when the proven bound closes the gap, otherwise `feasible` with the bound, and every point is checked against the original rows; `tools/verify_solution.py` reads `QCMATRIX` too. Off by default until measured on `main`. |
| **MIQP bound quality** - a QP interior point as the node solver is #494 | MIQP is implemented, but its node bound comes from a first-order method and is only accurate to the tolerance it converged to, so pruning is deliberately kept on the conservative side and costs nodes. With root cuts off by default too, expect incumbents more often than proofs. |
...
earned. A row whose issue is closed says *Landed* and stays as the record of what moved;
the open successors are the finale roadmap, #540, and each row names the ones that
apply to it.
...
|---|---|---|
| **CUDA backend for PDHG** | [#16](https://github.com/thegoodengineers/SANKHYA/issues/16), [#17](https://github.com/thegoodengineers/SANKHYA/issues/17), [#19](https://github.com/thegoodengineers/SANKHYA/issues/19); PR [#390](https://github.com/thegoodengineers/SANKHYA/pull/390) | The "GPU-accelerated" in the problem statement's title. The plumbing (#16) and the kernels (#17) are on `main` and compile in the CUDA CI job; the evidence (#19) is the CPU-vs-GPU crossover by model size and the honest 1e-8 list, which #390 measured on an RTX 5050 and committed, and #424 re-ran (`bench/results/gpu-fb72ab4.csv` is what section 1g of `docs/BENCHMARKS.md` now reads; `gpu-a4f02b1.csv` is #390's original and stays in the tree): PDHG alone on the solver's own clock, a warm-up solve absorbing CUDA's context creation, after a first attempt that had timed the interior-point polish and the process start-up along with the solve was sent back. The measurement protocol is in [`docs/GPU_PLAN.md`](docs/GPU_PLAN.md). #488's datacenter half ran the same three runners on a rented L4 from a fresh clone at `fdc89c5` (section 1g.3 of `docs/BENCHMARKS.md`): 4.21x at 10,000 x 10,000 at 1e-4, and the `gpu_datacenter.py` table of medians with the forced-iteration pair. The real-model runs were repeated on the L4 and an A100 from a fresh clone at `58a8374`, against both CPU arms (#675) and quoted against the faster: brazil3 5.75x / 8.10x at 1e-4 / 1e-8 on the L4 (one thread was faster than 16) and 8.91x / 10.53x on the A100 (`gpu-real-{l4,a100}-58a8374.csv`), with the GPU's answer further from HiGHS than the CPU's at 1e-8. H100 not yet. The first run of that card was thrown away because its CSVs were stamped `-dirty` (#583). |
...
| **Branch-and-cut proper** | [#221](https://github.com/thegoodengineers/SANKHYA/issues/221) | Mixed-integer rounding cuts, and cuts below the root. Root Gomory, cover and MIR cuts are on by default since `0018254`, once cut selection (#415) made the root round prove 10 of 30 where no cuts prove 9 at 1.015x the nodes (`bench/results/miplib-cuts-{off,on}.csv`); before selection the same round cost nodes and a match, and was off. Cut rounds below the root exist (`tree_cut_depth`) and stay opt-in: at depth 4 they lose the gained proof again in fewer, dearer nodes. What is left on this issue is the per-node cost of the waiting pool that makes tree rounds lose what the root gains. |
...
| **General NLP engine beside the `solve()` seam** | [#226](https://github.com/thegoodengineers/SANKHYA/issues/226), NLP stages 1-2 | A nonlinear model is read from AMPL's `.nl` format or built through the C API or Python (`sankhya.nonlinear`), with exact first and second derivatives by automatic differentiation. `sankhya solve model.nl` runs a primal-dual interior point with a filter line search after Wachter & Biegler (2006), with the inertia certified by the project's own LDL^T. It reports `optimal` only when the model is proved convex, `locally_optimal` for any other KKT point, and `locally_infeasible` when it finds a local minimizer of the violation; `tools/verify_solution.py` checks a `.nl` answer with its own reader and derivatives. On the branch, 64 of 70 Hock-Schittkowski problems reach the published optimum and all 70 answers verify; no results CSV from `main` yet. A model with integer columns goes to NLP-based branch and bound (stage 3), run only when the relaxation is proved convex and refused otherwise: on the branch, 14 of 17 convex MINLPLib instances are proved optimal at their published values, and the other three are reported honestly as not proved (a node relaxation hit the NLP iteration limit). What remains: a results CSV from a `main` build, the NLP engine's robustness on badly scaled models, and outer approximation (#528). |
