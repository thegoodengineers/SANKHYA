...
import qplib_doc  # the convex continuous QPLIB section (#492), kept in its own file
...
def main() -> int:
    ...
    # Only a run over every fetched instance is qplib-<sha>.csv; a subset run is
    # qplib-partial-<sha>.csv and the prefix filter keeps it out (#492).
...
## 2c. Convex continuous QPLIB subset

The convex continuous subset of QPLIB (Furini et al., *QPLIB: a library of quadratic
programming instances*, Mathematical Programming Computation 11, 2019): instances where the
objective type is C (convex QP) and the variable type is C (continuous), fetched by
`bench/runners/fetch_qplib.py` from https://qplib.zib.de/. Solved by the default QP engine
and judged on primal residual, dual residual and duality gap at 1e-6 and 1e-9, against the
published reference objectives and by the independent verifier.

{qplib_doc.section(qplib_csv)}
---

## 2d. The standard pooling instances, non-convex
