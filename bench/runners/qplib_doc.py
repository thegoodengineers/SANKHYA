...
"""The QPLIB convex continuous section of docs/BENCHMARKS.md, rendered from its CSV (#492).

Kept out of make_benchmarks_doc.py, which calls section() and is already several times the
project's per-file size, and importable on its own so tests can render a section from a
synthetic CSV without generating the whole document.
...
def failure_lists(rows: list[dict]) -> list[str]:
    """One line per way of not succeeding, each naming its instances."""
    lines = []
    groups = [
        ...
        ("`optimal` but more than 1e-6 relative from the reference objective",
         [r for r in rows if r.get("status") == "optimal"
          and r.get("matches_reference") != "1"]),
        ...
        ("`optimal` but not successful at 1e-6 relative",
         [r for r in rows if r.get("status") == "optimal"
          and r.get("success_rel_1e-6") != "1"]),
        ("`optimal` but not successful at 1e-9 relative",
         [r for r in rows if r.get("status") == "optimal"
          and r.get("success_rel_1e-9") != "1"]),
    ]
...
def section(path: Path | None) -> str:
    ...
    out = [
        ...
        "",
        "| level | success, relative measures (#492) | success, absolute measures |",
        "|---|---:|---:|",
        ...
        "residual and the duality gap are all within it, each recomputed from the QPLIB file "
        "and the written `.sol` by `bench/runners/qp_residuals.py`. The reference objectives "
        "are from the QPLIB website and Furini et al. (Math. Programming Computation 11, "
        "2019), parsed by `bench/runners/fetch_qplib.py` into `data/qplib/reference.json` "
        "with every file's sha256.",
        "",
        f"Shifted geometric mean of solver time, shift {SHIFT_SECONDS:g} s, an instance not "
        f"successful at 1e-6 charged the full limit: "
