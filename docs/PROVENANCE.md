...
| **Stub, not implemented yet:** `src/gpu/pdhg_graph.cu` has no kernel and returns without doing the work; the option is accepted and does nothing. Planned: GPU PDHG on-device iteration loop via CUDA Graphs (#478, off by default): K=64 iterations captured as a CUDA Graph and replayed with one launch, eliminating the per-iteration host round-trip; step-size rule and KKT residuals evaluated on-device; one small D→H copy per block | Lu & Yang, *cuPDLP.jl*, arXiv:2311.12180; NVIDIA, *CUDA C++ Programming Guide*, chapter "CUDA Graphs" | `src/gpu/pdhg_graph.cu` |
...
| GPU interior-point via cuDSS (#489, off by default, licence pending): normal equations factored by NVIDIA cuDSS on the device; Newton iteration on host unchanged; dense-column split per #467 before factorization | Shin et al., *MadIPM GPU interior point*, arXiv:2508.16094; NVIDIA cuDSS documentation (licence check required before this merges — see judgement call row 15 in section 5) | `src/gpu/ipm_cudss.cpp` |
...
| **Stub, not implemented yet:** `src/gpu/domain_prop.cu` has no kernel and returns without doing the work; the option is accepted and does nothing. Planned: GPU activity-based bound propagation (#510, off by default): activity bounds per row computed by one CUDA thread per row; variable bounds tightened by reversing each variable's contribution; iterated to convergence | Sofranac, Gleixner & Pokutta, *Accelerating domain propagation: an efficient GPU implementation for solving mixed-integer programs*, arXiv:2009.07785 | `src/gpu/domain_prop.cu` |
| **Stub, not implemented yet:** `src/gpu/gpu_heuristics.cu` has no kernel and returns without doing the work; the option is accepted and does nothing. Planned: GPU feasibility pump (#509, off by default): PDHG on the device as the LP projection step; outer loop rounds to integer and repeats until feasible | Fischetti, Glover & Lodi, *Feasibility pump*, Mathematical Programming 104 (2005); Mexi et al., *Combining Dual Simplex and GPU PDLP*, arXiv:2307.03466; Corduk et al., *GPU Heuristics for MILP*, arXiv:2510.20499 | `src/gpu/gpu_heuristics.cu` |
| **Stub, not implemented yet:** `src/gpu/gpu_heuristics.cu` has no kernel and returns without doing the work; the option is accepted and does nothing. Planned: GPU fix-and-propagate (#509, off by default): fixes near-integer columns from LP relaxation, propagates bounds on GPU, solves residual LP | Corduk et al., arXiv:2510.20499 | `src/gpu/gpu_heuristics.cu` |
| **Stub, not implemented yet:** `src/gpu/gpu_fj.cu` has no kernel and returns without doing the work; the option is accepted and does nothing. Planned: GPU Feasibility Jump (#508, off by default): activities and column weights on the device; jump value and score evaluated per candidate by one warp with a segmented CUB reduction; several independent restarts as parallel CUDA blocks | Corduk, Sielski, Boucher & Aatish, *GPU-accelerated primal heuristics for mixed integer programming*, arXiv:2510.20499 | `src/gpu/gpu_fj.cu` |
...
| Halpern-restarted first-order QP with PID primal weight (#493, off by default): Halpern anchor term on the Condat-Vu step; Q x once per outer iteration; PID controller updates sigma each restart; GPU path via cuSPARSE | Lu & Yang, *A practical and optimal first-order method for large-scale convex quadratic programming* (PDQP), arXiv:2311.07710; HPR-QP, arXiv:2507.02470; PDHCG-II, arXiv:2602.23967 | `src/pdhg/pdhg_qp_halpern.cpp` |
