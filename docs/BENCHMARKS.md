...
**Flow cover cuts (#419), on versus off** (`bench/results/miplib-419-cuts-baseline.csv` and `miplib-419-flow-cover-on.csv`, both at `91b6c08`, `enable_root_cuts=true` in both legs, 30 instances, 60 s, `mip_threads=1`): three MIPLIB instances share a fixed-charge / single-node flow row structure - a continuous flow variable switched on by a binary through a variable upper bound - that none of the existing families separates (Padberg, Van Roy & Wolsey, *Valid linear inequalities for fixed charge problems*, Operations Research 33 (1985)); `flow_cover_cuts.cpp` implements that family, gated behind a new `enable_flow_cover_cuts` option, validated against a closed-form textbook case and a 1,500-model exact-rational-arithmetic enumeration sweep (`tests/unit/test_flow_cover_cuts.cpp`), and routed through the same density/dynamism/duplicate filter as every other family. The family fires on real instances - 14 candidates on `ran12x21` alone, moving its root bound from 3157.38 to 3228.32 - but at the default root-only round (`tree_cut_depth=0`) it does not close enough of the gap in 60 s to change the outcome anywhere in the set: **13 of 30 reach the published optimum and 9 prove it, identical to the baseline with the family off.** None of the three target instances moves to the published optimum: `ran12x21` 3832 -> 3853 (worse), `ran13x13` 3319 -> 3297 (closer, still short of 3252), `k16x240b` 12271 -> 12271 (unchanged); no proof is lost or gained anywhere in the set. A diagnostic run with `tree_cut_depth=4` (cut rounds below the root, as the existing MIR/combinatorial measurement above also tried) made every one of the three worse rather than better - the extra rows cost node-LP time the family's own violation does not repay at this separation strength. **Acceptance criterion is not met**: no target instance reaches its published optimum under this measurement, so `enable_flow_cover_cuts` stays off by default per the same "measurement, not caution" reasoning as `enable_root_cuts` itself. Strengthening the separation (more than the three greedy orderings tried per row, or the single-node relaxation across several rows the issue names as a follow-up) is the next lever, not a change to the inequality or the filter.

**Single-node flow relaxation, on top of the above** (`bench/results/miplib-419-aggregation-baseline.csv` and `miplib-419-aggregation-on.csv`, both at `ebafccf`, same conditions as above): the follow-up the previous paragraph named. `flow_cover_cuts.cpp` now aggregates in another, not-yet-used row - the same elimination `mir_cuts.cpp` uses for its own continuous-variable aggregation, retargeted here at whichever single column keeps a capacity row from being a clean single-node flow row (typically a shared arc variable between two implied network nodes) - up to a depth of 3, and separates on the resulting aggregate at every depth tried. Two focused tests cover it directly: a hand-built two-node network where neither row alone has enough capacity to form a cover but the aggregate does (`AggregationAcrossTwoRowsFindsACoverNeitherRowHasAlone`), validated by projecting the eliminated shared variable's now-zero column out and reusing the existing exact worst-case-vertex argument on the aggregate (sound because the aggregate is an inequality *implied* by the two original rows, hence a relaxation of the true feasible set - a cut valid there is valid on the true set too); and a five-row chain one elimination longer than the depth cap allows, checked to abort cleanly rather than loop or read out of bounds (`AggregationStopsAtTheDepthLimitRatherThanLoopingForever`). The existing textbook case, negative control and 1,500-model sweep all still pass unchanged.

On the MIPLIB 30, the result is unchanged from the paragraph above: **13 of 30 reach the published optimum and 9 prove it, identical with aggregation on or off.** The reason is directly measurable, not a guess: the new `aggregated_cuts`/`deepest` stats (logged alongside the existing candidate count) read **zero** on `ran12x21`, `ran13x13` and `k16x240b` specifically - their fixed-charge rows are already clean single-node flow rows without any aggregation, so this extension had no opportunity to fire on the three target instances at all. The one-row separator was already the right shape for them; the gap that remains is separation strength on an already-qualifying row (more elaborate cover search, or the exact sequential lifting of Gu, Nemhauser & Savelsbergh (1999) that flow_cover_cuts.hpp already names as future work), not row-shape incompleteness. `ran12x21` moves 3812 -> 3853 (worse), `ran13x13` moves 3319 -> 3297 (closer, still short of 3252), `k16x240b` is unchanged at 12271; no proof lost or gained anywhere in the 30-instance set either way. **Acceptance criterion remains not met**, so `enable_flow_cover_cuts` stays off by default.

**A bounded local search over the cover choice, on top of both of the above** (`bench/results/miplib-419-localsearch-baseline.csv` and `miplib-419-localsearch-on.csv`, both at `8358f75`, same conditions as above): exact separation of the flow cover polytope is NP-hard - a candidate cover's violation depends on its own excess, which is a function of the whole set, so it does not decompose per item the way a textbook knapsack DP needs - and the three greedy orderings were only ever prefixes of a sort, which cannot reach every cover. `flow_cover_cuts.cpp` now runs a bounded, deterministic local search on top of the best prefix cover: flip one item's membership at a time, in a fixed column order, keep the flip only when it strictly improves the violation and is still a valid cover, up to two full sweeps. On the 30-instance set this reached **15 of 30 matched (up from 13) and 9 of 30 proved (unchanged)**, gaining `neos-3611689-kaihu` and `neos5`. That gain does not survive a same-commit re-check, though: a repeated baseline run (flow cover OFF, identical binary) also matched `neos-3611689-kaihu` and additionally *lost* `noswot`'s match relative to every earlier baseline in this section - both symptoms of ordinary run-to-run timing variance in a 60 s wall-clock budget (reliability branching and the primal heuristics both make timing-sensitive choices), not a code effect. The one comparison that IS robust across all four rounds run for #419 - one-row separation, single-node aggregation, and this local search, each checked against its own same-commit baseline - is the three ORIGINALLY TARGETED instances: `ran12x21`, `ran13x13` and `k16x240b` never once reach their published optimum, in any of the eight A/B legs run. **Acceptance criterion remains not met** on the instances #419 names, so `enable_flow_cover_cuts` stays off by default; whatever small overall benefit the family may be carrying elsewhere in the set is real but too close to this benchmark's own noise floor to claim from a single 60 s pass.
