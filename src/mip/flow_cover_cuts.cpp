...
std::vector<Cut> generate_flow_cover_cuts(const Model& model, const Solution& solution,
  ...
  for (Index i = 0; i < model.num_rows(); ++i) {
    ...
    for (int side = 0; side < 2; ++side) {
      ...
      const double b = sign * bound;

      // STRICT SCOPE (flow_cover_cuts.hpp): every nonzero of the row must be a qualifying flow
      // term, positively oriented, with a variable upper bound found for it. A single foreign
      // column (a plain integer, an unlinked continuous variable, or a negative-oriented
      // coefficient) means this is not the single-node flow set the theorem is stated for, and
      // the row/side is skipped rather than guessed at.
      std::vector<FlowTerm> terms;
      terms.reserve(row.columns.size());
      bool qualifies = true;
      for (std::size_t k = 0; k < row.columns.size() && qualifies; ++k) {
        const double coef = sign * row.values[k];
        if (std::fabs(coef) <= tol::kZeroDrop) continue;
        const Index col = row.columns[k];
        const auto uj = static_cast<std::size_t>(col);
        if (coef <= 0.0 || model.col_type[uj] == VarType::kInteger || !vubs[uj].has_value()) {
          qualifies = false;
          break;
        }
        FlowTerm term;
        ...
        term.effective_capacity = coef * vubs[uj]->capacity;
        term.z_star = coef * solution.col_value[uj];
        term.y_star = solution.col_value[static_cast<std::size_t>(term.binary_col)];
        terms.push_back(term);
      }
      if (!qualifies || terms.size() < 2) continue;

      double total_capacity = 0.0;
      for (const FlowTerm& t : terms) total_capacity += t.effective_capacity;
      if (total_capacity - b <= kMinExcess) continue;  // no cover possible

      // Three deterministic orderings: capacity, LP flow, and LP activation, each descending.
      // Any resulting prefix with positive excess is a valid cover (flow_cover_cuts.hpp); the
      // orderings only change whether the search happens to find a violated one.
      ...
      std::sort(by_capacity.begin(), by_capacity.end(), [&](std::size_t a, std::size_t b2) {
      ...
      for (const auto& order : {by_capacity, by_flow, by_binary}) {
        auto result = best_cover_in_order(terms, order, b);
        if (result && (!best || result->violation > best->violation)) best = std::move(result);
      }
      if (!best) continue;

      std::vector<char> in_cover(terms.size(), 0);
      for (const std::size_t member : best->members) in_cover[member] = 1;
      ...
      for (std::size_t k = 0; k < terms.size(); ++k) {
        ...
        const double slack_capacity = t.effective_capacity - best->excess;
        if (slack_capacity > tol::kZeroDrop) {
      ...
      // z_j := sign * a_j * x_j was a direct substitution, not a flip of the inequality's
      // direction, so t.coefficient (= sign * a_j) is already the correct coefficient on the
      // ORIGINAL x_j in the final "<=" cut.

      // LIFTING (partial, see flow_cover_cuts.hpp): a leftover flow column's own variable
      // upper bound (coefficient * x_j - effective_capacity * y_j <= 0) is a valid inequality
      // in its own right; adding any non-negative multiple to a valid inequality keeps it
      // valid. It is folded in exactly when doing so costs nothing at the current point - the
      // bound is already tight there - so the round's separation never loses violation by
      // carrying the extra columns.
      ...
      for (std::size_t k = 0; k < terms.size(); ++k) {
        ...
        // A non-negative multiple (t.coefficient, > 0) of the column's own, always-valid
        // variable upper bound x_j - capacity * y_j <= 0, added directly in ORIGINAL
        // coordinates (the row-sense flip above has already been undone, so no further sign
        // handling belongs here - the VUB itself never had a "sense" to flip).
        cut.coeff[static_cast<std::size_t>(t.flow_col)] += t.coefficient;
        cut.coeff[static_cast<std::size_t>(t.binary_col)] -= t.effective_capacity;
        lifted_any = true;
        if (stats != nullptr) ++stats->lifted_terms;
      }
      ...
      if (stats != nullptr) {
        ++stats->cuts;
        if (lifted_any) ++stats->lifted_cuts;
      }
