...
bool upload_vec(T** dst, const T* src, std::size_t n) {
  if (cudaMalloc(reinterpret_cast<void**>(dst), (n == 0 ? 1 : n) * sizeof(T)) != cudaSuccess)
    return false;
  if (n > 0 && cudaMemcpy(*dst, src, n * sizeof(T), cudaMemcpyHostToDevice) != cudaSuccess)
    return false;
  return true;
}
...
__global__ void k_primal_update(double* X, const double* ATY, const double* col_cost,
                                const double* lb_block, const double* ub_block, int n, int K,
                                double tau) {
...
std::vector<BatchNodeResult> solve_batch_nodes(const Model& model, int K,
  ...
  const int nnz = static_cast<int>(model.matrix.num_nonzeros());
  ...
  std::vector<double> val(static_cast<std::size_t>(nnz));
  {
    ...
    for (int i = 0; i <= m; ++i)
      row_ptr[static_cast<std::size_t>(i)] = static_cast<int>(rs[static_cast<std::size_t>(i)]);
    for (int p = 0; p < nnz; ++p) {
      col_idx[static_cast<std::size_t>(p)] = static_cast<int>(ci[static_cast<std::size_t>(p)]);
      val[static_cast<std::size_t>(p)] = cv[static_cast<std::size_t>(p)];
    }
  }
  ...
  for (int i = 0; i < m; ++i) {
    ...
    if (std::isinf(lo))
      rhs[static_cast<std::size_t>(i)] = hi;
    else if (std::isinf(hi))
      rhs[static_cast<std::size_t>(i)] = lo;
    else
      rhs[static_cast<std::size_t>(i)] = 0.5 * (lo + hi);
  }
  ...
  double* d_row_lo = nullptr;
  double* d_row_hi = nullptr;
  ...
  DeviceMat d_lb, d_ub;
  if (!X.alloc(n, K) || !Y.alloc(m, K) || !AX_scratch.alloc(m, K) || !ATY_scratch.alloc(n, K) ||
             ...
             static_cast<std::size_t>(m) * static_cast<std::size_t>(K) * sizeof(double));
  {
    ...
    for (int k = 0; k < K; ++k) {
      for (int j = 0; j < n; ++j) {
        const double lo = col_lb[static_cast<std::size_t>(k) * static_cast<std::size_t>(n) +
                                 static_cast<std::size_t>(j)];
        const double hi = col_ub[static_cast<std::size_t>(k) * static_cast<std::size_t>(n) +
                                 static_cast<std::size_t>(j)];
        const double mid = (std::isinf(lo) ? 0.0 : (std::isinf(hi) ? lo : 0.5 * (lo + hi)));
        x_init[static_cast<std::size_t>(j) +
               static_cast<std::size_t>(k) * static_cast<std::size_t>(n)] = mid;
      }
    }
  ...
  cusparseCreateCsr(&mat, static_cast<int64_t>(m), static_cast<int64_t>(n),
                    static_cast<int64_t>(nnz), d_row_ptr, d_col_idx, d_val, CUSPARSE_INDEX_32I,
  ...
  cusparseCreateDnMat(&dm_ATY, static_cast<int64_t>(n), static_cast<int64_t>(K),
                      static_cast<int64_t>(n), ATY_scratch.data, CUDA_R_64F,
                      CUSPARSE_ORDER_COL);

  // SpMM workspace.
  std::size_t ws1 = 0, ws2 = 0;
  cusparseSpMM_bufferSize(cs, CUSPARSE_OPERATION_NON_TRANSPOSE,
                          CUSPARSE_OPERATION_NON_TRANSPOSE, &kOne, mat, dm_X, &kZero, dm_AX,
                          CUDA_R_64F, CUSPARSE_SPMM_ALG_DEFAULT, &ws1);
  ...
  std::vector<BatchNodeResult> results(
  ...
  for (int iter = 0; iter < max_iters && active_count > 0; ++iter) {
    ...
    cusparseSpMM(cs, CUSPARSE_OPERATION_NON_TRANSPOSE, CUSPARSE_OPERATION_NON_TRANSPOSE, &kOne,
                 mat, dm_X, &kZero, dm_AX, CUDA_R_64F, CUSPARSE_SPMM_ALG_DEFAULT, ws);
    ...
    {
      const dim3 grid((m + n_threads - 1) / n_threads, K);
      k_dual_update<<<grid, n_threads>>>(Y.data, AX_scratch.data, d_row_lo, d_row_hi, m, K,
                                         sigma);
    }
    ...
    cusparseSpMM(cs, CUSPARSE_OPERATION_TRANSPOSE, CUSPARSE_OPERATION_NON_TRANSPOSE, &kOne, mat,
                 dm_Y, &kZero, dm_ATY, CUDA_R_64F, CUSPARSE_SPMM_ALG_DEFAULT, ws);
    // Step 5: primal update X -= tau * (ATY - c), clip to bounds.
    {
      const dim3 grid((n + n_threads - 1) / n_threads, K);
      k_primal_update<<<grid, n_threads>>>(X.data, ATY_scratch.data, d_cost, d_lb.data,
                                           d_ub.data, n, K, tau);
    }
  ...
  // Cleanup.
  if (ws) cudaFree(ws);
  cusparseDestroyDnMat(dm_X);
  cusparseDestroyDnMat(dm_AX);
  cusparseDestroyDnMat(dm_Y);
  cusparseDestroyDnMat(dm_ATY);
