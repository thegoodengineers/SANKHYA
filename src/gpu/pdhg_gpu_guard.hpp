...
/// refused on more than one device (the multi-GPU engine's atomicAdd reductions are
/// order-dependent, #383; the single-device engine is repeatable since #478), the device's
/// compute capability must meet the minimum this build was compiled for (#282), and the
/// estimated device memory must fit in what is free (#281). Any failure logs why and returns
/// false; the caller then runs CPU PDHG instead. A query that itself fails (no device, driver
